import { requiresHumanGate } from "./operator";

export type LicenceTier = {
  key: "tier_1_pilot" | "tier_2_single_domain" | "tier_3_multi_domain" | "tier_4_strategic";
  label: string;
  price_gbp: string;
  term: string;
  scope: string;
...
export const LICENCE_TIERS: LicenceTier[] = [
  {
    key: "tier_1_pilot",
    label: "Tier 1 Pilot",
    price_gbp: "5000",
...
export function getLicenceTier(key: LicenceTier["key"]): LicenceTier {
  const tier = LICENCE_TIERS.find((t) => t.key === key);
  if (!tier) throw new Error(`Unknown licence tier: ${key}`);
  return tier;
}

export function createLicenceOffer(key: LicenceTier["key"]) {
