export type OpportunityCategory = "safe" | "scalable" | "experimental";

export type MatrixOpportunity = {
  name: string;
  category: OpportunityCategory;
  deploymentType: "service_arbitrage" | "digital_product" | "marketplace_flip";
  description: string;
  expectedROI: number;
...
function scoreOpportunity(item: Omit<MatrixOpportunity, "score">): number {
...
type Signal = { problem: string; frequency: number; sentiment: number; velocity: number };

function matchSignal(problem: string, keyword: string): boolean {
  return String(problem || "").toLowerCase().includes(String(keyword || "").toLowerCase());
}

function signalFor(opportunityName: string, signals: Signal[]) {
  ...
  for (const s of rows) {
    const p = String(s.problem || "").toLowerCase();
    if (
      ...
      totalFreq += Number(s.frequency || 0);
      totalVelocity += Number(s.velocity || 0);
      hits += 1;
    }
  }
...
export function generateOpportunityMatrix(capital: number, signals: Signal[] = []): MatrixOpportunity[] {
  const c = Math.max(0, Number(capital || 0));
  const base: Array<Omit<MatrixOpportunity, "score" | "dynamicPriority" | "signalStrength" | "demandVelocity">> = [
    {
