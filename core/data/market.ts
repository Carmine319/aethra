import type { MarketRegion, RealSignal } from "../types";
...
export function detectMarketRegion(): MarketRegion {
  const r = String(process.env.AETHRA_MARKET_REGION || process.env.VERCEL_REGION || "UK")
    .toUpperCase()
    .slice(0, 8);
  if (r.includes("US") || r === "IAD1" || r === "SFO1") return "US";
  if (r.includes("EU") || r.includes("FRA") || r.includes("AMS")) return "EU";
  if (r.includes("UK") || r.includes("LHR")) return "UK";
  return "OTHER";
}
 ...
 */
export function demandElasticityProxy(signal: RealSignal): number {
 ...
 */
export function pricingSensitivity(region: MarketRegion): number {
  switch (region) {
    case "US":
      return 0.62;
    case "EU":
...
export function applyRegionalScoreWeight(score: number, region: MarketRegion): number {
