...
exports.assessCapitalRisk = assessCapitalRisk;
function parseTimeToCashDays(v) {
...
function assessCapitalRisk(selected, template, portfolio) {
    ...
    const timeToCashDays = Math.max(7, Number(template.time_to_revenue_days) || parseTimeToCashDays(payload.time_to_cash));
    const downsideRisk01 = Math.min(0.95, riskFromProfile(String(template.risk_profile || "medium")) +
